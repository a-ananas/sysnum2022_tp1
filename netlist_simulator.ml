...
let mux ch a b = match (ch, a, b) with
	...
	|_ -> raise (Fail "weid thing in mux... no arrays for choice... yet")
...
let slice i j arg = 
	...
	then raise(Fail "Wrong indices for slicing")
		...
		|(VBit _) -> raise(Fail "Slicing a bit is impossible... or is it ?")

let select i arg = match arg with
	|(VBitArray arr) -> (VBit(Array.get arr i))
	|(VBit _) when i = 0 -> arg
	|_ -> raise (Fail "thing went wrong around slice") 
